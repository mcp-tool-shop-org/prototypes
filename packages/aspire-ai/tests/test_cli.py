...
def test_train_with_demo_prompts():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            patch("aspire.config.AspireConfig", return_value=mock_config) as mock_config_class,
...
def test_evaluate_with_mock_checkpoint():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Patch at the aspire.config and aspire.trainer module level
        with (
            patch("aspire.config.AspireConfig") as mock_config_class,
            patch("aspire.trainer.AspireTrainer", return_value=mock_trainer),
        ):
            mock_config_class.from_yaml.return_value = mock_config
...
def test_diagnose_text():
    """Test aspire diagnose outputs diagnostics table."""
    result = runner.invoke(app, ["diagnose"])
    assert result.exit_code == 0
    assert "python_version" in result.stdout
    assert "aspire_version" in result.stdout
    assert "All checks passed" in result.stdout


def test_diagnose_json():
    """Test aspire diagnose --json outputs valid JSON."""
    result = runner.invoke(app, ["diagnose", "--json"])
    assert result.exit_code == 0
    # Extract JSON from Rich output (may have ANSI codes)
    # The JSON output comes from console.print_json which prints clean JSON
    assert '"ok"' in result.stdout
    assert '"checks"' in result.stdout


def test_diagnose_checks_version():
