...
class MCPToolGrader:
    ...
    def __init__(
        ...
        Args:
            ...
            plugin_registry: Optional plugin registry with custom rules
        """
        self.strict_security = strict_security
        self.include_optional_checks = include_optional_checks
        self._plugin_registry = plugin_registry

        # Collect plugin patterns
        plugin_patterns = plugin_registry.get_all_threat_patterns() if plugin_registry else []

        # Initialize validators
        self.validator = MCPToolValidator(strict_mode=False, check_security=True)
        self.security = SecurityScanner(
            fail_on_medium=strict_security,
    ...
    def grade(self, tool: dict[str, Any]) -> GradeReport:
        ...
        # Run all validators
        validation_result = self.validator.validate(cast("MCPToolDefinition", tool))
        security_result = self.security.scan(tool)
        compliance_result = self.compliance.check(tool)

        # Run plugin compliance checks
        if self._plugin_registry:
            self._run_plugin_compliance_checks(tool, compliance_result)
            self._run_plugin_quality_checks(tool, validation_result)
    ...
    def _run_plugin_compliance_checks(
        self,
        tool: dict[str, Any],
        report: ComplianceReport,
    ) -> None:
        """Run plugin compliance checks and append to the report."""
        if not self._plugin_registry:
            return
        for spec in self._plugin_registry.get_all_compliance_checks():
            try:
                status, message, details = spec.check_fn(tool)
                report.checks.append(
                    ComplianceCheck(
                        ...
                        message=message,
                        details=details,
                        spec_reference=spec.spec_reference,
                    )
                )
            except Exception:
                pass  # Skip broken plugin checks

    def _run_plugin_quality_checks(
        self,
        tool: dict[str, Any],
        result: ValidationResult,
    ) -> None:
        """Run plugin quality checks and adjust score."""
        if not self._plugin_registry:
            return
        for spec in self._plugin_registry.get_all_quality_checks():
            try:
                deduction, message = spec.check_fn(tool)
                if deduction > 0 and message:
                    result.score = max(0.0, result.score - deduction)
