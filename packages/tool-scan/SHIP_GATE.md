...
- [x] `[all]` No telemetry by default — state it explicitly even if obvious (2026-02-27 — "collects nothing, all local and offline")

### Default safety posture

- [x] `[cli|mcp|desktop]` SKIP: no dangerous actions — read-only scanner, no kill/delete/restart operations
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27 — reads only JSON files passed as arguments, writes nothing)
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27 — GradeReport with score, grade, remarks, is_safe; Remark with category, severity, title, detail, fix)
- [x] `[cli]` Exit codes: 0 ok · 1 user error · 2 runtime error · 3 partial success (2026-02-27 — 0 passed, 1 failed, 2 error)
- [x] `[cli]` No raw stack traces without `--debug` (2026-02-27 — --verbose flag for details, otherwise clean output)
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
...
- [x] `[cli|mcp|desktop]` Logging levels defined: silent / normal / verbose / debug — secrets redacted at all levels (2026-02-27 — default clean, --verbose for detail, --json for automation)
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: straightforward scanner, not complex enough for HANDBOOK
...
- [x] `[pypi]` Clean wheel + sdist build (2026-02-27 — hatchling build, OIDC publish workflow)
