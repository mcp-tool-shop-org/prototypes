...
def _mock_run_async(return_value):
    """Create a mock run_async that closes the coroutine to avoid warnings."""
...
class TestDepsCLI:
    ...
    def test_tree_json(self) -> None:
        ...
        with patch(
            "headless_wheel_builder.depgraph.cli.run_async", side_effect=_mock_run_async(mock_graph)
        ):
