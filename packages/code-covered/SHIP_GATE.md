...
- [x] `[all]` No telemetry by default — state it explicitly even if obvious (2026-02-27 — zero runtime deps, no network)

### Default safety posture

- [x] `[cli|mcp|desktop]` SKIP: no dangerous actions — read-only analysis, output only via explicit -o flag
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27 — reads coverage.json + source, writes only via -o flag)
- [ ] `[mcp]` SKIP: MCP adapter is optional, not a standalone server
- [ ] `[mcp]` SKIP: MCP adapter is optional, not a standalone server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27 — structured JSON output, prioritized suggestions)
- [x] `[cli]` Exit codes: 0 ok · 1 user error · 2 runtime error · 3 partial success (2026-02-27 — 0 success, 1 error)
...
- [x] `[cli|mcp|desktop]` SKIP: single-command CLI, no logging levels needed — errors go to stderr
- [ ] `[mcp]` SKIP: MCP adapter is optional, not a standalone server
- [ ] `[complex]` SKIP: straightforward CLI tool, not complex enough for HANDBOOK
...
- [x] `[all]` Dependency scanning runs in CI (ecosystem-appropriate) (2026-02-27 — pip-audit in dependency-audit job)
