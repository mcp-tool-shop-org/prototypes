...
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app — pure computation library
...
- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — typed error classes in DSP package
...
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app
- [ ] `[mcp]` SKIP: not an MCP server
- [x] `[complex]` HANDBOOK.md: daily ops, warn/critical response, recovery procedures (2026-02-27) — Reference_Handbook.md + extensive docs/ directory

## D. Shipping Hygiene

- [x] `[all]` `verify` script exists (test + build + smoke in one command) (2026-02-27) — `npm test` + `npm run smoke`
...
- [x] `[npm]` `npm pack --dry-run` includes: dist/, README.md, CHANGELOG.md, LICENSE (2026-02-27) — files array in package.json
- [x] `[npm]` `engines.node` set (2026-02-27) — Node >=20 per badge
