...
describe("validateCapture node validation", () => {
  ...
  it("node with __proto__ key → rejected", () => {
    // Simulate a crafted JSON payload with prototype pollution attempt
  ...
  it("parseCapture rejects __proto__ at top level", () => {
    // Guard in parseCapture scrubs __proto__ from the top-level parsed object
    const json = '{"__proto__":{"polluted":true},"version":"0.1","url":"http://x","timestamp_ms":1,"viewport":{"w_px":1,"h_px":1,"aspect":1},"compiler":{"name":"websketch-ir","version":"0.1","options_hash":"x"},"root":{"id":"","role":"PAGE","bbox":[0,0,1,1],"interactive":false,"visible":true}}';
    // Should not throw due to pollution — the guard deletes the key
...
describe("parseCapture", () => {
  ...
  it("invalid JSON → throws WS_INVALID_JSON", () => {
    ...
    try {
      ...
      expect((err as WebSketchException).name).toBe("WebSketchException");
