...
- [x] `[cli|mcp|desktop]` SKIP: no dangerous actions — health-checks only, quarantine is reversible (auto-restore)
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27 — writes only .mcp.json and .mcp.health.json in project dir)
- [ ] `[mcp]` SKIP: not an MCP server (it health-checks MCP servers, doesn't serve as one)
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27 — health_check returns {name, healthy, reason}, run returns {status, message, healthy, quarantined})
- [x] `[cli]` Exit codes: 0 ok · 1 user error · 2 runtime error · 3 partial success (2026-02-27 — 0 ok, 1 unknown command)
- [x] `[cli]` No raw stack traces without `--debug` (2026-02-27 — clean stderr logging via _log())
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
...
- [x] `[cli]` `--help` output accurate for all commands and flags (2026-02-27 — usage printed on unknown command)
- [x] `[cli|mcp|desktop]` Logging levels defined: silent / normal / verbose / debug — secrets redacted at all levels (2026-02-27 — single-level stderr logging, no secrets to redact)
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: simple health-check tool, not complex enough for HANDBOOK
...
- [x] `[pypi]` Clean wheel + sdist build (2026-02-27 — setuptools backend)
