...
- [ ] `[cli|mcp|desktop]` SKIP: no dangerous actions — read-only scanning, reports written to .venvkit/
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27 — writes only to .venvkit/ output dir)
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27 — FindingCode enum with 19 codes, severity levels, fix suggestions)
...
- [x] `[cli]` `--help` output accurate for all commands and flags (2026-02-27 — venvkit-map --help)
- [ ] `[cli|mcp|desktop]` SKIP: single-level logging via console output
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: straightforward diagnostic tool, not complex enough for HANDBOOK
...
- [x] `[all]` Dependency scanning runs in CI (ecosystem-appropriate) (2026-02-27 — npm audit + dependency-review-action)
