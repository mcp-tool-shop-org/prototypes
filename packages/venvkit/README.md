...
| `artifact` | Build artifact or output |
...
| `CREATED_FROM` | venv creation provenance |
| `ROUTES_TASK_TO` | task → env routing |
| `FAILED_RUN` | task → env failure (dashed in Mermaid) |
| `SHARES_WHEELHOUSE` | environments sharing a wheel cache |
| `SHADOWS_PATH` | path shadowing between environments |
