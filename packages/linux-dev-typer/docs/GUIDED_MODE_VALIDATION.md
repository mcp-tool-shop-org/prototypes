...
- [ ] Fresh install or reset persisted state
- [ ] At least 10 snippets available in the current language
- [ ] Debug inspector accessible via Ctrl+Shift+D (Debug builds)
...
| # | Step | Expected |
|---|------|----------|
| 1.1 | Launch app, open Settings | Guided Mode toggle is OFF |
| 1.2 | Complete 3 practice sessions | Pick reasons show standard selection (no "weakness" reasons) |
| 1.3 | Open debug inspector (Ctrl+Shift+D) | SignalPolicy shows `GuidedMode=false`, `EffectiveSelectionBias=false` |
...
| # | Step | Expected |
|---|------|----------|
| 3.1 | With Guided Mode ON, mistype `{`, `}`, `(`, `)` heavily | HeatmapCategoryBias applied to selections |
| 3.2 | Check debug inspector scoring | `HeatmapCategoryBias` ≤ +3 for any snippet |
| 3.3 | Complete 10 sessions | Still see variety of snippets (not all bracket-heavy) |
| 3.4 | Only 1 weak symbol category (e.g., only CurlyBraces) | Diversity guard: bias shows 0 (needs 2+ categories) |

**Pass criteria**: Bias is capped at +3, diversity guard requires 2+ weak categories.
...
| 6.2 | Drill snippets target top weakness | Drill description mentions weak symbol category |
