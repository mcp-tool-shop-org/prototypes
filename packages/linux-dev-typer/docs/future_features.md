...
Features that were removed, deferred, or imagined during v1.0 development.
None of these are committed — they exist so good ideas don't get buried.

---

## Removed in v1.0 (Could Return)

### Practice Intent Labels
User-declared labels (Warmup, Explore, Drill, Challenge) attached to sessions.
...
**If restored:** Make intent _mean_ something. A "Drill" intent could auto-select
from top weakness categories. A "Warmup" could pick easy comfort-zone snippets.
A "Challenge" could push +1 difficulty above comfort zone. The infrastructure
is still in the codebase (PracticeIntent enum, SessionMetadata.Intent field,
InsightEngine intent-aware messages).
...
168 calibration snippets exist across 5 languages (python, rust, javascript,
...
(e.g., "Python list comprehensions", "Rust ownership patterns").
