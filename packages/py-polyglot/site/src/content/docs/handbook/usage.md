...
Cache files are stored as `.polyglot-cache.json` next to the source file when you provide the `file_path` option. Without `file_path`, caching is in-memory only and does not persist to disk.

```python
# Persistent cache (saved to disk next to README.md)
result = await translate_markdown(md, "en", "ja",
    TranslateMarkdownOptions(file_path="README.md"))

# Clear an existing cache for a file
result = await translate_markdown(md, "en", "ja",
    TranslateMarkdownOptions(file_path="README.md", cache_clear=True))
```

Cache entries expire after 30 days. The fuzzy matching threshold is 85% similarity (Levenshtein-based).
