...
4. **validate.py** checks the response for quality issues (echo detection, truncation, garbled text, meta-commentary)
5. **polish.py** strips alternative translations (detected via multilingual "or" patterns), trailing periods from headings, excessive whitespace, and line-edge spaces
...
104 tests across 12 test files covering all modules. Tests use pytest with pytest-asyncio for async test support.
