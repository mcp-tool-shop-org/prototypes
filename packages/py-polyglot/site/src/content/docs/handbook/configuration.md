...
Translation cache stores segment-level results in `.polyglot-cache.json`, saved next to the source file when `file_path` is provided in `TranslateMarkdownOptions`. Without `file_path`, caching is in-memory only and does not persist across runs.

The cache uses a custom Levenshtein similarity function for fuzzy matching (threshold: 85%). Segments that are similar enough to a cached entry reuse the cached translation. Cache entries expire after 30 days.

To clear a persisted cache, delete the cache file:

```bash
rm .polyglot-cache.json
```

Or pass `cache_clear=True` in options:

```python
from pypolyglot import translate_markdown, TranslateMarkdownOptions

result = await translate_markdown(md, "en", "ja",
    TranslateMarkdownOptions(file_path="README.md", cache_clear=True))
```
