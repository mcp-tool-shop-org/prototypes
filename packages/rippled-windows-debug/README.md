...
Apply the patch to `src/xrpld/app/main/Main.cpp` (a ready-made patch is at `patches/rippled_main.patch`):
...
Compile with `-DRIPPLED_WINDOWS_DEBUG` to activate the toolkit. Without the define, the include is skipped entirely.
...
├── tools/
│   └── build-governor/     # Automatic OOM protection (.NET 9.0)
│       └── src/
│           ├── Gov.Service/      # Background service (token pool, memory monitor)
│           ├── Gov.Wrapper.CL/   # cl.exe wrapper (requests tokens)
│           ├── Gov.Wrapper.Link/ # link.exe wrapper (requests tokens)
│           ├── Gov.Common/       # Shared client, memory metrics, autostart
│           ├── Gov.Protocol/     # Message types for pipe communication
│           └── Gov.Cli/          # CLI tool (gov run, gov status)
├── scripts/
│   ├── setup-governor.ps1  # One-command governor build + PATH setup
│   ├── build-rippled.ps1   # Full rippled build pipeline with governor
│   └── get_git_info.bat    # Batch script for git compiler defines
├── cmake/
│   └── GitInfo.cmake       # CMake module for git defines
├── patches/
│   └── rippled_main.patch  # Patch for Main.cpp
├── examples/
│   └── test_crash.cpp      # Demo program (7 test modes)
├── docs/
