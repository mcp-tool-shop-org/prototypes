...
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app — data-only npm package
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app
- [ ] `[mcp]` SKIP: not an MCP server — metadata-only registry
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27 — build scripts use process.exit with descriptive messages; schema validation emits structured AJV errors)
...
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app
- [ ] `[mcp]` SKIP: not an MCP server
- [x] `[complex]` HANDBOOK.md: daily ops, warn/critical response, recovery procedures (2026-02-27 — HANDBOOK.md already exists)
...
- [x] `[all]` Automated dependency update mechanism exists (2026-02-27 — husky + lint-staged on commit)
- [x] `[npm]` `npm pack --dry-run` includes: dist/, README.md, CHANGELOG.md, LICENSE (2026-02-27 — verify-pack script enforces this)
