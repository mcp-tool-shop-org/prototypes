...
- [ ] `[cli|mcp|desktop]` SKIP: CLI has no dangerous actions — governance is read-evaluate only
...
- [ ] `[cli|mcp|desktop]` SKIP: uses Node.js debug module, no custom logging levels needed
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: not a complex ops tool — extensive docs already (ARCHITECTURE, PLUGIN_GUIDE, etc.)
