...
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app — Claude Code plugin only
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app
- [ ] `[mcp]` SKIP: not an MCP server (wraps one via npx)
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — plugin delegates to underlying MCP server
...
- [ ] `[cli|mcp|desktop]` SKIP: not a CLI, MCP server, or desktop app
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: simple plugin — not complex enough for handbook

## D. Shipping Hygiene

- [x] `[all]` `verify` script exists (test + build + smoke in one command) (2026-02-27) — plugin is config-only, no build step
