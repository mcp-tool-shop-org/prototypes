...
- [x] `[cli|mcp|desktop]` Dangerous actions (kill, delete, restart) require explicit `--allow-*` flag — SKIP: testing toolkit sends read-only MCP requests; no destructive actions
- [x] `[cli|mcp|desktop]` File operations constrained to known directories — writes JSON reports to output directory only
- [ ] `[mcp]` SKIP: CLI tool, not an MCP server (tests MCP servers but is not one itself)
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` — structured JSON error output
...
- [x] `[cli|mcp|desktop]` Logging levels defined: silent / normal / verbose / debug — --json for machine output, stderr for diagnostics
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: profiles + schemas provide operational reference

## D. Shipping Hygiene

- [x] `[all]` `verify` script exists (test + build + smoke in one command) — tests/ directory
...
- [ ] `[npm]` SKIP: Python project
- [ ] `[npm]` SKIP: Python project
- [ ] `[pypi]` SKIP: no pyproject.toml — Python CLI bundled in engines/ directory
