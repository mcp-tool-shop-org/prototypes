...
- [x] `[all]` No secrets, tokens, or credentials in source or diagnostics output (2026-02-27) — evidence redaction strips tokens before writing
- [x] `[all]` No telemetry by default — state it explicitly even if obvious (2026-02-27)

### Default safety posture

- [x] `[cli|mcp|desktop]` Dangerous actions (kill, delete, restart) require explicit `--allow-*` flag (2026-02-27) — read-only tool, no destructive actions
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27) — writes only to --output dir and --cache-dir
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — 30+ namespaced error codes (COE.*)
- [x] `[cli]` Exit codes: 0 ok · 1 user error · 2 runtime error (2026-02-27)
...
- [ ] `[mcp]` SKIP: not an MCP server
- [x] `[complex]` HANDBOOK.md: daily ops, warn/critical response, recovery procedures (2026-02-27) — RUNBOOK.md with complete error reference
...
- [x] `[npm]` `npm pack --dry-run` includes: src/, schema/, README.md, CHANGELOG.md, LICENSE (2026-02-27)
- [x] `[npm]` `engines.node` set (2026-02-27) — >=20
