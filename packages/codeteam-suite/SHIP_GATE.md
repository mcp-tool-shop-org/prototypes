...
- [ ] `[cli|mcp|desktop]` SKIP: CLI has no dangerous actions — verification is read-only, signing writes only to package directories
...
- [ ] `[cli|mcp|desktop]` SKIP: CLI uses structured exit codes rather than logging levels
- [ ] `[mcp]` SKIP: not an MCP server
