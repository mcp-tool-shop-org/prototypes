...
- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — ErrorInfo interface + classifyError()
- [ ] `[cli]` SKIP: not a CLI tool
- [ ] `[cli]` SKIP: not a CLI tool
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[desktop]` SKIP: not a desktop app
- [x] `[vscode]` Errors surface via VS Code notification API — no silent failures (2026-02-27) — friendlyError() with action buttons
...
- [ ] `[cli]` SKIP: not a CLI tool
- [ ] `[cli|mcp|desktop]` SKIP: VS Code extension — logging via Output Channel, no configurable levels
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: simple extension, not complex enough for ops handbook
