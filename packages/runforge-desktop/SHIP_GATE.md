...
- [x] `[cli|mcp|desktop]` Dangerous actions (kill, delete, restart) require explicit `--allow-*` flag — SKIP: ML training runs require explicit user action to start/cancel
- [x] `[cli|mcp|desktop]` File operations constrained to known directories — user-selected workspace only
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` — .NET exception hierarchy
...
- [x] `[desktop]` Installer/package builds and runs on stated platforms — MSIX for Windows 10/11
