...
- [x] `[cli|mcp|desktop]` Dangerous actions (kill, delete, restart) require explicit `--allow-*` flag (2026-02-27) — fail-safe: if governor unavailable, wrappers run tools ungoverned
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27) — named-pipe IPC only, no file writes beyond process state
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — OOM failure classification with structured diagnostics
- [x] `[cli]` Exit codes: 0 ok · 1 user error · 2 runtime error · 3 partial success (2026-02-27) — gov CLI returns structured exit codes
- [x] `[cli]` No raw stack traces without `--debug` (2026-02-27) — user-friendly error messages
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
- [x] `[desktop]` Errors shown as user-friendly messages — no raw exceptions in UI (2026-02-27) — service logs structured messages
...
- [x] `[all]` `verify` script exists (test + build + smoke in one command) (2026-02-27) — dotnet build + dotnet test in CI
- [x] `[all]` Version in manifest matches git tag (2026-02-27)
- [x] `[all]` Dependency scanning runs in CI (ecosystem-appropriate) (2026-02-27) — NuGet central package management
- [x] `[all]` Automated dependency update mechanism exists (2026-02-27)
- [ ] `[npm]` SKIP: .NET project, not npm
- [ ] `[npm]` SKIP: .NET project, not npm
- [ ] `[npm]` SKIP: .NET project, not npm
- [ ] `[vsix]` SKIP: not a VS Code extension
- [x] `[desktop]` Installer/package builds and runs on stated platforms (2026-02-27) — NuGet packages + CLI publish on Windows
