...
// Validates the structural shape of a CrawlResult without replicating every nested type.
// The formatters already handle missing/malformed fields defensively.
...
const ExportDataSchema = z.union([
  CrawlResultShape,
  z.array(CrawlResultShape).min(1),
]).describe('Previously crawled repo data (CrawlResult or array of CrawlResults)');
    ...
    async (args) => {
      try {
        ...
        switch (args.format) {
          ...
          case 'csv':
            return successResponse(exportCSV(data, args.sections));

          case 'markdown':
            return successResponse(exportMarkdown(data, args.sections));
