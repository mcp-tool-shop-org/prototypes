...
- **10 MCP tools** — crawl repos, crawl orgs, search, read files, commit diffs, workflow runs, summarize, compare, export, cache management
- **3-tier data model** — start light, go deep when you need to
- **In-memory response cache** — TTL-based caching saves API quota on repeated calls; per-data-type TTLs (2–10 min)
- **Pagination** — `page` param on `search_repos`, `crawl_org`, and `get_workflow_runs` for browsing large result sets
...
```
crawl_org({ org: "vercel", tier: "1", min_stars: 100, language: "TypeScript", concurrency: 5 })
```
...
```
search_repos({ query: "mcp server", language: "typescript", min_stars: 50, sort: "stars" })
```
...
```
get_file_content({ owner: "facebook", repo: "react", path: "package.json" })
```
...
```
get_commit_diff({ owner: "facebook", repo: "react", ref: "abc123" })
```
...
Get recent CI/CD workflow run results. Returns status, conclusion, duration, branch, and event trigger.

```
get_workflow_runs({ owner: "facebook", repo: "react", limit: 5 })
```
...
```
search_repos({ query: "mcp server", language: "typescript", min_stars: 10, sort: "stars", limit: 20 })
```
...
### Review a commit
```
get_commit_diff({ owner: "myorg", repo: "api", ref: "HEAD" })
```

### Check CI health
```
get_workflow_runs({ owner: "myorg", repo: "api", limit: 5 })
```

### Fast org scan with concurrency
```
crawl_org({ org: "myorg", tier: "1", concurrency: 5, repo_limit: 50 })
```
...
- **Response cache** — TTL expiry, hit/miss tracking, prune, clear, key generation
- **GitHub adapter** — Tier 1/2/3 fetching, search, file content, commit diffs, workflow runs, caching, section filtering, error handling, permission tracking
- **Tool schemas** — Zod validation for all 10 tools, param defaults, pagination, edge cases
...
src/
  ...
  tools/
    ...
    crawlOrg.ts         # crawl_org — concurrent org-wide crawling with filters
    searchRepos.ts      # search_repos — global GitHub search
    getFileContent.ts   # get_file_content — fetch file from a repo
    getCommitDiff.ts    # get_commit_diff — commit diff with patches
    getWorkflowRuns.ts  # get_workflow_runs — recent CI/CD run results
    clearCache.ts       # clear_cache — cache management
...
- **Intelligent caching** — In-memory TTL cache with per-data-type expiry. Immutable data (commits) cached 10 min, fast-moving data (issues) cached 2 min. `clear_cache` tool for manual invalidation.
